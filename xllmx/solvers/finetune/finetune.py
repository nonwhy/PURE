    ...
    def build_model(self) -> (nn.Module, Tokenizer):
        ...
        if hasattr(unwrapped_model, "get_trainable_params"):
            ...
            if self.args.apply_lora:
                self.logger.info("LoRA applied to the model")
                for key, param in unwrapped_model.named_parameters():
                    if "lora_" not in key:
                        param.requires_grad = False
                        promote_param_to_fp32(param)
                        ...
                        param.requires_grad = True
                        promote_param_to_fp32(param)
                self.logger.info("Non-LoRA parameters frozen")
            else:
                self.logger.warning(
        ...
        # # 冻结除embedding和lm_head外的所有参数
        # for name, param in unwrapped_model.named_parameters():
        #     if not any(x in name for x in ['embed_tokens', 'lm_head']):
        #         param.requires_grad = False
        ...
        # ----------------
        self.is_lora = getattr(unwrapped_model, "is_lora", False)  # todo
        self.logger.info(f"Model is lora: {self.is_lora}")
        # ----------------
    ...
    def train_one_epoch(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            with {
                ...
                c_loss, additional_loss_dict = self.model(input_ids=examples, labels=labels)
            loss = c_loss
